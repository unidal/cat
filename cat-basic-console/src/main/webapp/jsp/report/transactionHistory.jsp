	...
	<c:choose>
		<c:when test="${empty payload.type}">
			...
			<c:forEach var="machine" items="${model.report.machines}">
				<c:forEach var="type" items="${machine.value.types}" varStatus="status">
					<c:set var="e" value="${type.value}"/>
					<c:set var="lastIndex" value="${status.index}" />
					<tr class="${status.index mod 2 != 0 ? 'odd' : 'even'}">
						<td style="text-align: left">
							<a href="?op=historyGraph&domain=${model.domain}&date=${model.date}&ip=${model.ipAddress}&reportType=${model.reportType}&type=${e.id}${model.customDate}" class="history_graph_link" data-status="${status.index}">[:: show ::]</a>
							<a href="?op=history&domain=${model.domain}&date=${model.date}&ip=${model.ipAddress}&reportType=${model.reportType}&type=${e.id}${model.customDate}">${e.id}</a>
						</td>
		...
		<c:otherwise>
			...
			<tr>
				<th><a href="?op=history&domain=${model.domain}&date=${model.date}&ip=${model.ipAddress}&reportType=${model.reportType}&type=${payload.type}&sort=type${model.customDate}&queryname=${model.queryName}">Name</a></th>
			...
			<c:forEach var="machine" items="${model.report.machines}">
				<c:forEach var="type" items="${machine.value.types}">
					<c:forEach var="name" items="${type.value.names}" varStatus="status">
