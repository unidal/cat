...
public class Handler implements PageHandler<Context> {
	...
	private void buildHistoryGraph(Model model, Payload payload) {
		HeartbeatReport report = m_reportService.queryHeartbeatReport(payload.getDomain(), new Date(payload.getDate()), new Date(
		      payload.getDate() + TimeUtil.ONE_HOUR));
	...
	private HeartbeatReport getReport(String domain, String ipAddress, long dateLong, ModelPeriod period) {
		...
		if (m_service.isEligable(request)) {
			...
			if (period.isLast()) {
				Set<String> domains = m_reportService.queryAllDomainNames(new Date(dateLong), new Date(dateLong + TimeUtil.ONE_HOUR),
				      "heartbeat");
